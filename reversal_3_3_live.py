...
from backtest_reversal_3_1_calls import PreparedHistory
from backtest_reversal_3_3_timing_overlay_experiment import (
    add_timing_indicators,
    build_candidate_dataset,
    fit_timing_model_asof,
    score_live_feature_row,
)
...
LIVE_DIR = BASE_DIR / "results" / "reversal_3_3_live"
ROOT_README_PATH = BASE_DIR / "README.md"
ROOT_SECTION_START = "<!-- reversal-3.3-live:start -->"
ROOT_SECTION_END = "<!-- reversal-3.3-live:end -->"
...
PLOT_1D_PATH = ASSETS_DIR / "reversal_3_3_live_equity_1d.png"
PLOT_1W_PATH = ASSETS_DIR / "reversal_3_3_live_equity.png"
PLOT_1M_PATH = ASSETS_DIR / "reversal_3_3_live_equity_1m.png"
PLOT_OVERALL_PATH = ASSETS_DIR / "reversal_3_3_live_equity_overall.png"
TIMING_MODEL_PATH = LIVE_DIR / "timing_overlay_model.pkl"
TIMING_MODEL_META_PATH = LIVE_DIR / "timing_overlay_model_meta.json"
BENCHMARK_COLORS = {
    "QQQ": "#f59e0b",
    "SPY": "#93c5fd",
}
...
def build_prepared_history_from_csv(ticker: str) -> PreparedHistory | None:
    df = load_reversal_csv(ticker).copy()
    if "Close" not in df.columns:
        df["Close"] = df["Adj Close"]
    df["Date"] = pd.to_datetime(df["Date"], errors="coerce").dt.normalize()
    df["Sigma Ann"] = pd.to_numeric(df["Rolling Sigma 20d"], errors="coerce")
    future_high = pd.concat(
        [pd.to_numeric(df["High"].shift(-i), errors="coerce") for i in range(1, FORWARD_DAYS + 1)],
        axis=1,
    ).max(axis=1)
    df["Future High"] = future_high
    df["Signal Drop Amount"] = pd.to_numeric(df["Max Drop"], errors="coerce") * pd.to_numeric(df["Prev Close"], errors="coerce")
    df["Recover Ratio"] = (pd.to_numeric(df["Future High"], errors="coerce") - pd.to_numeric(df["Low"], errors="coerce")) / df["Signal Drop Amount"]
...
def build_timing_history_map(tickers: list[str]) -> dict[str, PreparedHistory]:
    histories: dict[str, PreparedHistory] = {}
    for ticker in tickers:
        try:
            prepared = build_prepared_history_from_csv(ticker)
        except Exception:
            prepared = None
...
def build_live_timing_feature_row(
    df: pd.DataFrame,
    live: dict[str, Any],
    success_rate: float,
    matched_signals: int,
    ...
    session_high = float(live.get("session_high", np.nan)) if pd.notna(live.get("session_high")) else current_price
    session_low = float(live.get("session_low", np.nan)) if pd.notna(live.get("session_low")) else current_price

    working = df.copy()
    latest_date = pd.Timestamp(now_et).tz_convert(ET).normalize().tz_localize(None)
    live_row = {
        "Date": latest_date,
        "Open": prev_close,
        "High": max(session_high, current_price, prev_close),
        "Low": min(session_low, current_price, prev_close),
        "Close": current_price,
        "Adj Close": current_price,
        "Prev Close": prev_close,
        "Max Drop": float(live["current_drop_pct"]),
    }
    if not working.empty and pd.Timestamp(working["Date"].iloc[-1]).normalize() == latest_date:
        for key, value in live_row.items():
            working.at[working.index[-1], key] = value
    else:
        working = pd.concat([working, pd.DataFrame([live_row])], ignore_index=True)

    working["Date"] = pd.to_datetime(working["Date"], errors="coerce").dt.normalize()
    working["Log Return"] = np.log(pd.to_numeric(working["Adj Close"], errors="coerce") / pd.to_numeric(working["Adj Close"], errors="coerce").shift(1))
    working["Rolling Sigma 20d"] = working["Log Return"].rolling(ROLLING_SIGMA_WINDOW).std() * np.sqrt(252)
    working = add_timing_indicators(working, (TIMING_OVERLAY_WINDOW,))
    row = working.iloc[-1]
    feature_row = {
        "success_rate": float(success_rate),
        "matched_signals": int(matched_signals),
        "current_drop_pct": float(live["current_drop_pct"]),
        "rolling_sigma_20d_pct": float(row["Rolling Sigma 20d"]) * 100 if pd.notna(row["Rolling Sigma 20d"]) else np.nan,
    }
    for feature, value in row.items():
        if str(feature).startswith(("sma_gap_", "ema_gap_", "atr_pct_", "admi_", "cci_", "roc_", "rsi_", "willr_", "stoch_k_", "stoch_d_", "close_location_")):
            feature_row[str(feature)] = float(value) if pd.notna(value) else np.nan
    return feature_row


def load_cached_timing_model(now_et: pd.Timestamp) -> Any | None:
    if not TIMING_MODEL_PATH.exists() or not TIMING_MODEL_META_PATH.exists():
        return None
    try:
        meta = json.loads(TIMING_MODEL_META_PATH.read_text())
    except Exception:
        return None
    if meta.get("trade_date_et") != now_et.date().isoformat():
        return None
    try:
        with TIMING_MODEL_PATH.open("rb") as fh:
            return pickle.load(fh)
    except Exception:
        return None


def save_cached_timing_model(now_et: pd.Timestamp, model: Any, training_samples: int) -> None:
    LIVE_DIR.mkdir(parents=True, exist_ok=True)
    with TIMING_MODEL_PATH.open("wb") as fh:
        pickle.dump(model, fh)
    TIMING_MODEL_META_PATH.write_text(
        json.dumps(
            {
                "trade_date_et": now_et.date().isoformat(),
                "window": TIMING_OVERLAY_WINDOW,
                "threshold": TIMING_OVERLAY_THRESHOLD,
...
def get_or_build_timing_model(tickers: list[str], now_et: pd.Timestamp) -> tuple[Any | None, dict[str, Any]]:
    cached = load_cached_timing_model(now_et)
    if cached is not None:
        meta = json.loads(TIMING_MODEL_META_PATH.read_text())
        return cached, {"status": "cached", **meta}

    history_map = build_timing_history_map(tickers)
    candidate_start = (pd.Timestamp(now_et.date()) - pd.DateOffset(years=TIMING_OVERLAY_RESEARCH_YEARS)).normalize()
    candidate_df = build_candidate_dataset(history_map, candidate_start)
    model = fit_timing_model_asof(candidate_df, TIMING_OVERLAY_WINDOW, pd.Timestamp(now_et.date()))
    if model is None:
        return None, {
            "status": "unavailable",
            "reason": "insufficient_training_data",
            "candidate_events": int(len(candidate_df)),
        }
    save_cached_timing_model(now_et, model, len(candidate_df))
    return model, {
        "status": "built",
        "trade_date_et": now_et.date().isoformat(),
        "window": TIMING_OVERLAY_WINDOW,
...
def screen_live_candidates(tickers: list[str], now_et: pd.Timestamp | None = None) -> tuple[pd.DataFrame, dict[str, pd.DataFrame], dict[str, Any]]:
    ...
    if asof_clock.tzinfo is None:
        asof_clock = asof_clock.tz_localize(ET)
    else:
        asof_clock = asof_clock.tz_convert(ET)

    timing_model, timing_meta = get_or_build_timing_model(tickers, asof_clock)
    ...
    for ticker in tickers:
        try:
            ...
            if np.isnan(success_rate):
                timing_score = np.nan
                timing_status = "no_signal_history"
            else:
                feature_row = build_live_timing_feature_row(
                    df=df,
                    live=live,
                    success_rate=float(success_rate),
                ...
                timing_score = score_live_feature_row(feature_row, timing_model, TIMING_OVERLAY_WINDOW)
                if not np.isfinite(timing_score):
                    timing_status = "model_unavailable"
                elif timing_score >= TIMING_OVERLAY_THRESHOLD:
                    timing_status = "pass"
                else:
...
def maybe_enter_position(
        ...
        try:
            ...
            if not option_liquid:
                events_df = append_event(
                    ...
                    trade_date=now_et.date().isoformat(),
                    slot=slot_key,
                    event_type="entry_candidate_skipped",
                    detail={
                        "ticker": ticker,
    ...
    events_df = append_event(
        ...
        slot=slot_key,
        event_type="entry_skipped",
        detail={"reason": "no_trade_after_option_and_timing_filters"},
    )
    return state, trades_df, events_df
...
def build_chart_sections(image_prefix: str, cache_bust: str | None = None) -> list[str]:
    ...
    return [
        ...
        "",
        f"![Reversal {VERSION} Live Equity Overall]({image_prefix}assets/reversal_3_3_live_equity_overall.png{suffix})",
        "",
        "</details>",
        "",
        "<details>",
        ...
        "",
        f"![Reversal {VERSION} Live Equity 1D]({image_prefix}assets/reversal_3_3_live_equity_1d.png{suffix})",
        "",
        "</details>",
        "",
        "<details>",
        ...
        "",
        f"![Reversal {VERSION} Live Equity 1W]({image_prefix}assets/reversal_3_3_live_equity.png{suffix})",
        "",
        "</details>",
        "",
        "<details>",
        ...
        "",
        f"![Reversal {VERSION} Live Equity 1M]({image_prefix}assets/reversal_3_3_live_equity_1m.png{suffix})",
        "",
        "</details>",
        "",
    ]
...
def maybe_git_publish(now_et: pd.Timestamp, dry_run: bool, skip_git_publish: bool) -> None:
    ...
    tracked_paths = [
        "README.md",
        "results/reversal_3_3_live/README.md",
        "results/reversal_3_3_live/live_trades.csv",
        "results/reversal_3_3_live/live_events.csv",
        "results/reversal_3_3_live/live_positions.csv",
        "results/reversal_3_3_live/live_equity.csv",
        "results/reversal_3_3_live/state.json",
        "results/reversal_3_3_live/timing_overlay_model_meta.json",
        "assets/reversal_3_3_live_equity.png",
        "assets/reversal_3_3_live_equity_1d.png",
        "assets/reversal_3_3_live_equity_1m.png",
        "assets/reversal_3_3_live_equity_overall.png",
    ]
...
def run_cycle(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if pre_start or closure_detail is not None:
        ...
        summary_df, _, timing_meta = screen_live_candidates(universe, now_et=now_et)
    else:
        summary_df = pd.DataFrame()
