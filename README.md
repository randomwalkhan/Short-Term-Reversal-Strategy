...
- Latest checkpoint (ET): `2026-04-13 15:30:05 EDT`
- Equity: `$13,167.24` | Realized: `$3,145.00` | Unrealized: `$22.24` | Open positions: `1`
...
ticker asset_type execution_mode instrument  units  cash_spent  current_position_value  current_price  unrealized_pnl  unrealized_return_pct  business_days_held
  REGN      share share_fallback       REGN      8      5916.0                 5938.24         742.28           22.24                   0.38                   0
```

<details open>
<summary><strong>Overall</strong></summary>

![Reversal 3.3 Live Equity Overall](assets/reversal_3_2_live_equity_overall.png?v=20260413153005)
...
![Reversal 3.3 Live Equity 1D](assets/reversal_3_2_live_equity_1d.png?v=20260413153005)
...
![Reversal 3.3 Live Equity 1W](assets/reversal_3_2_live_equity.png?v=20260413153005)
...
![Reversal 3.3 Live Equity 1M](assets/reversal_3_2_live_equity_1m.png?v=20260413153005)
...
Display update: GitHub feature charts stay on the shared dark Apple-style plotting theme, and the repo-facing version label is now aligned to Reversal 3.3.
...
更新说明：Reversal 3.3 保留 Reversal 3.1 的研究与交易口径不变，并在已加入的 NYSE 节假日保护、期权流动性门槛和 share fallback 基础上，把 option spread 门槛从 `12%` 放宽到 `15%`。live 仍要求 `open interest >= 100` 与 `volume >= 10`，但对可接受的 bid/ask 宽度更宽容一些。

展示更新：GitHub 上的主要曲线图继续沿用统一的深色 Apple Stocks 风格配色，对外版本标签现已统一为 Reversal 3.3。
...
The official Reversal 3.3 backtest definition keeps the Reversal 3.1 research setup intact, including the dynamic trade-level filter `matched_signals >= 10`, the promoted `60d` historical lookback window, the `minimum current drop > 0.5%` entry filter, and the curated universe overlay `qqq_plus_leverage_etfs = qqq_only_filtered + SOXL + UPRO`. On the current data snapshot, the official Reversal 3.3 result remains `+1709.09%` total return, `-44.30%` max drawdown, `63.33%` win rate, and `4.28` Sharpe.

Reversal 3.3 的官方回测定义保留了 Reversal 3.1 的研究与执行口径不变，包括动态交易级过滤 `matched_signals >= 10`、提升后的 `60d` 历史观察窗口、`minimum current drop > 0.5%` 入场过滤，以及精选的 universe overlay：`qqq_plus_leverage_etfs = qqq_only_filtered + SOXL + UPRO`。在当前数据快照下，官方 Reversal 3.3 结果仍为：总收益 `+1709.09%`、最大回撤 `-44.30%`、胜率 `63.33%`、Sharpe `4.28`。
...
- [Reversal 3.3 equity](results/reversal_3_1/reversal_3_1_call_backtest_equity.csv)
- [Reversal 3.3 trades](results/reversal_3_1/reversal_3_1_call_backtest_trades.csv)
- [Reversal 3.3 plot](assets/reversal_3_1_call_backtest_equity.png)

![Reversal 3.3 Call Backtest](assets/reversal_3_1_call_backtest_equity.png)
...
- `3.3`: keep the `3.2` research configuration unchanged, but relax the live option spread gate from `<= 12%` to `<= 15%` while keeping the same open-interest / volume checks and share-fallback logic
...
3. Run `backtest_reversal_3_1_calls.py` for the official Reversal 3.3 call backtest under `qqq_plus_leverage_etfs + matched_signals >= 10 + 60d + minimum current drop > 0.5%`.  
   如果你想跑官方 Reversal 3.3 主回测，再运行 `backtest_reversal_3_1_calls.py`；这部分使用 `qqq_plus_leverage_etfs + matched_signals >= 10 + 60d + minimum current drop > 0.5%`。
...
- `reversal_3_2_live.py` | Reversal 3.3 live paper-test runner with scheduled entry/exit logic, state persistence, dashboard generation, optional GitHub publishing, the promoted `qqq_plus_leverage_etfs` live universe, NYSE holiday protection, and the current option-liquidity/share-fallback execution path. | Reversal 3.3 的 live paper-test 主脚本，包含定时入场/离场逻辑、状态持久化、dashboard 生成、可选的 GitHub 发布、升级后的 `qqq_plus_leverage_etfs` live universe、NYSE 节假日保护，以及当前的期权流动性门槛与 share fallback 执行路径。
- `Reversal3.3.ipynb` | Current main notebook with the official `qqq_plus_leverage_etfs` universe, the default `60d` observation window, `minimum current drop > 0.5%` live-screen filter, improved extended-hours spot pricing, ATM-IV versus rolling-sigma context, and a cleaner single-table live screener output. | 当前主 notebook，使用官方 `qqq_plus_leverage_etfs` universe，默认 `60d` 观察窗口，加入 `minimum current drop > 0.5%` 的 live-screen 过滤，优先使用扩展时段 spot 价格，并在 screener 输出中补充 ATM IV 与 rolling sigma 对照，同时把 live screener 压缩成更清晰的单表输出。
- `Reversal3.2.ipynb` | Prior main notebook snapshot before the Reversal 3.3 live spread-gate update. | Reversal 3.3 放宽 live spread 门槛之前的上一版主 notebook 快照。
...
- `backtest_reversal_3_1_calls.py` | Official Reversal 3.3 call backtest with the curated `qqq_plus_leverage_etfs` overlay, the promoted `60d` observation window, and the `minimum current drop > 0.5%` filter. | Reversal 3.3 的官方 call 回测脚本，使用精选的 `qqq_plus_leverage_etfs` overlay、提升后的 `60d` 观察窗口和 `minimum current drop > 0.5%` 过滤。
...
- `qqq_plus_leverage_etfs_tickers.csv` | Saved ticker list for the promoted Reversal 3.3 universe overlay. | Reversal 3.3 官方 overlay universe 的保存版 ticker 列表。
