...
- `reversal_3_4_1_alpaca_paper.py` | Separate Alpaca paper execution runner that reuses the Reversal 3.4.1 signal stack, submits Alpaca paper option limit orders, reconciles fills, manages exits, and stores credentials only in a local ignored `.secrets/` file. | 独立的 Alpaca paper execution runner，复用 Reversal 3.4.1 信号栈，提交 Alpaca paper 期权 limit order，跟踪成交和退出，并且只从本地忽略提交的 `.secrets/` 目录读取凭据。
