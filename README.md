...
1. Loads a **local static ticker universe** from `us_tickers_static.txt` (offline-friendly; no Wikipedia dependency).
...
### Common Notebook Runtime Issue

If Part 1 (`Success Odds Analysis`) raises `FileNotFoundError: Reversal.xlsx`, ensure your working directory contains `Reversal.xlsx`, or set `FILE_PATH` to an absolute path. In v2.0, the cell now tries multiple local candidate paths and prints the resolved path.
...
Note: Part 5 now uses local `us_tickers_static.txt` instead of live Wikipedia scraping.
