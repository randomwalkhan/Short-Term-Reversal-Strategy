...
- Latest checkpoint (ET): `2026-05-07 19:40:05 EDT`
- Equity: `$27,483.50` | Realized: `$17,583.50` | Unrealized: `$-100.00` | Open positions: `1`
...
ticker asset_type execution_mode         instrument  units  cash_spent  current_position_value  current_price  unrealized_pnl  unrealized_return_pct  business_days_held
   TXN     option         option TXN260618C00290000     10     13100.0                 13000.0           13.0          -100.0                  -0.76                   0
```

<details open>
<summary><strong>Overall</strong></summary>

![Reversal 3.3.2 Live Equity Overall](assets/reversal_3_3_live_equity_overall.png?v=20260507194005)
...
![Reversal 3.3.2 Live Equity 1D](assets/reversal_3_3_live_equity_1d.png?v=20260507194005)
...
![Reversal 3.3.2 Live Equity 1W](assets/reversal_3_3_live_equity.png?v=20260507194005)
...
![Reversal 3.3.2 Live Equity 1M](assets/reversal_3_3_live_equity_1m.png?v=20260507194005)
...
`Reversal3.3.ipynb` remains the current research notebook for short-term reversal analysis and option profitability confidence estimation; `3.3.2` is the current execution/filtering version.

`Reversal3.3.ipynb` 仍是当前研究型 notebook，用于短期反转研究和期权盈利概率评估；`3.3.2` 是当前执行层和过滤器版本。

## Strategy Summary | 策略总结

This repository studies a short-term reversal call-buying setup built around large intraday drawdowns, historical recovery probability, and staged optimization. The current official version is `Reversal 3.3.2`.

本仓库研究的是一套基于“日内大跌后短期反转”的 call 策略，通过历史反弹成功率和逐阶段优化来推进。目前官方版本是 `Reversal 3.3.2`。
...
Release notes (`3.3.2`)
- Keep the official `3.3.1` universe and timing-overlay execution path intact.
- Add a live-entry trend-health gate that blocks candidates already sitting in a short-term down channel.
- The gate uses 10-trading-day return, short-window log-price slope, price versus the 10-day lookback average, and consecutive lower-close streak; it allows sideways names and single-day pullbacks, but excludes patterns such as CMCSA's recent lower-close sequence.

版本说明（`3.3.2`）
- 保留 `3.3.1` 的官方股票池和 timing-overlay 执行路径。
- 新增 live-entry trend-health gate，用于排除已经处在短期下跌通道中的候选。
- 该过滤器结合 10 个交易日收益、短窗 log-price slope、当前价格相对 10 日观察均值的位置，以及连续 lower-close streak；横盘和单日回调仍可通过，但类似 CMCSA 这种连续 lower-close 的信号会被排除。

## Featured Result | 重点结果

The latest recorded official Reversal 3.3 backtest keeps the promoted `3.1` research stack intact, then adds a `5d` timing overlay with a `0.50` no-trade gate on top of the existing candidate selection. On the current 1-year data snapshot, that change lifts win rate and sharply compresses drawdown while keeping Sharpe slightly above the no-overlay baseline. The current recorded Reversal 3.3 result is `+690.85%` total return, `-26.42%` max drawdown, `69.23%` win rate, and `4.32` Sharpe. Reversal `3.3.2` keeps that strategy definition, preserves the high trailing P/E universe guard from `3.3.1`, and adds the live trend-health gate described above.

最近一次记录的 Reversal 3.3 官方回测保留了 `3.1` 提升后的核心研究口径，并在现有候选筛选之上新增 `5d` timing overlay 与 `0.50` no-trade gate。在当前的 1 年数据快照下，这一改动显著压低了最大回撤，同时提升了胜率，并让 Sharpe 仍然略高于无 overlay 的基线。当前记录的 Reversal 3.3 结果为：总收益 `+690.85%`、最大回撤 `-26.42%`、胜率 `69.23%`、Sharpe `4.32`。Reversal `3.3.2` 保留这套策略定义，延续 `3.3.1` 的高 trailing P/E 股票池保护，并加入上面说明的 live trend-health gate。
...
- `reversal_3_3_live.py` | Reversal 3.3.2 live paper-test runner with scheduled entry/exit logic, state persistence, dashboard generation, optional GitHub publishing, the promoted `qqq_plus_leverage_etfs` live universe with the non-ETF trailing P/E guard, NYSE holiday protection, option-liquidity gating, the promoted `5d` timing overlay, and the live trend-health down-channel gate. | Reversal 3.3.2 的 live paper-test 主脚本，包含定时入场/离场逻辑、状态持久化、dashboard 生成、可选的 GitHub 发布、加入非 ETF trailing P/E 保护的 `qqq_plus_leverage_etfs` live universe、NYSE 节假日保护、期权流动性门槛、正式提升后的 `5d` timing overlay，以及 live trend-health 下跌通道过滤。
