...
from backtest_reversal_2_3_1_calls import (
    BACKTEST_YEARS,
    INITIAL_CAPITAL,
    OUTPUT_DIR,
...
SUMMARY_PATH = OUTPUT_DIR / "reversal_2_3_1_universe_comparison.csv"
PLOT_PATH = OUTPUT_DIR / "reversal_2_3_1_universe_comparison.png"
...
def plot_comparison(equity_curves: dict[str, pd.DataFrame]) -> None:
    ...
    plt.title("Reversal 2.3.1 Universe Comparison")
    plt.xlabel("Date")
...
def main() -> None:
    ...
    for label, tickers in presets.items():
        ...
        equity_df.to_csv(OUTPUT_DIR / f"{label}_equity.csv", index=False)
